...
pSQUID <- read.csv("./TARGET_AML_LogNormalize_LogNormalize_pSQUID.csv")

# Extract patients IDs
TARGET.USI <- strsplit(pSQUID$X, "[.]")
TARGET.USI <- matrix(unlist(TARGET.USI), ncol=5, byrow=TRUE)
TARGET.USI <- as.data.frame(TARGET.USI)
pSQUID <- cbind.data.frame(TARGET.USI, pSQUID)
pSQUID <- pSQUID[,-c(1,2,4,5,6)]
colnames(pSQUID)[1] <- "TARGET.USI"
